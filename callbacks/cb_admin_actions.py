...
from db.commands import create_promocode, get_all_users, topup_balance
...
async def get_id(msg: Message, state: FSMContext, db_session: async_session):
    ...
    res, desc = await topup_balance(db_session, user_id, amount)
    if not res:
        await msg.answer(f"❌ Произошла ошибка: {desc}")
        await state.clear()
...
async def get_promo(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        "<b>Введите количество использований промокода</b>", parse_mode="html"
    )
...
async def get_max_uses(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        "<b>Теперь введите сумму промокода</b>", parse_mode="html"
    )
...
async def get_amount(
    ...
    try:
        ...
        await msg.answer(
            "<b>❌ Сумма должна быть числом!</b>", parse_mode="html"
        )
    data = await state.get_data()
    promo = data.get("promo")
    max_uses = data.get("max_uses")
    res, desc = await create_promocode(db_session, promo, max_uses, amount)
    if not res:
        await msg.answer(f"❌ Произошла ошибка: {desc}")
        await state.clear()
    await msg.answer(
    ...
    for user in users:
        try:
            ...
            if (
                ...
                print(
                    f"Пользователь {user.id} заблокировал бота или удалил чат"
                )
            failed_count += 1
        except Exception as e:
